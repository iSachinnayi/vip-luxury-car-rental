// ═══════════════════════════════════════════════
//  Brand/Category Client — Same features as All Cars
//  Filters · Pagination · Grid/List · Animations
// ═══════════════════════════════════════════════
    ...
    <div className="relative" ref={ref}>
      <button type="button" onClick={() => setOpen(!open)}
        className="group w-full flex items-center gap-2 px-3 py-2.5 rounded-xl
                   ...
                   focus:border-gold/50 focus:ring-1 focus:ring-gold/20 transition-all duration-300 outline-none cursor-pointer">
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: -6, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -6, scale: 0.96 }}
            ...
            className="absolute z-30 mt-1.5 left-0 right-0 min-w-[160px] rounded-xl border border-white/[0.08]
...
function Pagination({ current, total, onChange, labels }: {
  ...
  const getPages = () => {
    ...
    else { p.push(1); if (current > 3) p.push("..."); for (let i = Math.max(2, current - 1); i <= Math.min(total - 1, current + 1); i++) p.push(i); if (current < total - 2) p.push("..."); p.push(total); }
    return p;
  };
  return (
    <motion.nav initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
      ...
      <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} disabled={current === 1} onClick={() => onChange(current - 1)}
        className="w-9 h-9 md:w-10 md:h-10 rounded-xl flex items-center justify-center text-gray-500 hover:text-gold hover:bg-white/[0.04] border border-transparent hover:border-white/10 disabled:opacity-20 disabled:cursor-not-allowed transition-all duration-300" aria-label={labels.previous}>{Icons.chevronLeft}</motion.button>
      {getPages().map((page, i) => page === "..." ? <span key={"d-" + i} className="w-5 text-center text-gray-600 text-xs select-none">...</span>
        : <motion.button key={page} whileHover={page !== current ? { scale: 1.05 } : {}} whileTap={page !== current ? { scale: 0.95 } : {}} onClick={() => onChange(page)}
            className={"relative w-9 h-9 md:w-10 md:h-10 rounded-xl text-xs md:text-sm font-medium transition-all duration-300 " + (page === current ? "text-black bg-gold shadow-[0_4px_15px_rgba(200,169,81,0.3)]" : "text-gray-400 hover:text-white hover:bg-white/[0.04] border border-transparent hover:border-white/10")}
            aria-current={page === current ? "page" : undefined} aria-label={labels.page.replace("{page}", String(page))}>
      ...
      <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} disabled={current === total} onClick={() => onChange(current + 1)}
        className="w-9 h-9 md:w-10 md:h-10 rounded-xl flex items-center justify-center text-gray-500 hover:text-gold hover:bg-white/[0.04] border border-transparent hover:border-white/10 disabled:opacity-20 disabled:cursor-not-allowed transition-all duration-300" aria-label={labels.next}>{Icons.chevronRight}</motion.button>
    </motion.nav>
  );
}

// ─── List View Item ───
function ListCarItem({ car, index, fromLabel, perDayLabel, locale }: { car: CarCardData; index: number; fromLabel: string; perDayLabel: string; locale: string }) {
  const { symbol, fmt } = useCurrency();
  const [imgError, setImgError] = useState(false);
  const imgSrc = car.thumbnail || car.images?.[0] || "";
  return (
    <motion.a href={locale === "en" ? "/car/" + car.slug : "/" + locale + "/car/" + car.slug} initial={{ opacity: 0, x: -15 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
      transition={{ delay: index * 0.03 }} whileHover={{ x: 4 }}
      className="group flex items-center gap-4 p-3 rounded-2xl bg-white/[0.02] border border-white/[0.06] hover:bg-white/[0.04] hover:border-white/10 transition-all duration-300">
      <div className="w-24 h-16 md:w-32 md:h-20 rounded-xl overflow-hidden flex-shrink-0 bg-dark">
        {!imgError && imgSrc ? <img src={imgSrc} alt={car.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" onError={() => setImgError(true)} />
          : <div className="w-full h-full flex items-center justify-center text-gray-600"><svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M5 17h14M5 17a2 2 0 01-2-2V9a2 2 0 012-2h1l2-3h8l2 3h1a2 2 0 012 2v6a2 2 0 01-2 2M5 17v2a1 1 0 001 1h2a1 1 0 001-1v-2M17 17v2a1 1 0 001 1h2a1 1 0 001-1v-2"/></svg></div>}
      </div>
...
// ─── Main Component ───
export default function BrandCategoryClient({ slug, isBrand, pageTitle }: {
  slug: string; isBrand: boolean; pageTitle: string;
}) {
  ...
  const [selectedBrand, setSelectedBrand] = useState(isBrand ? getBrandFromSlug(slug) : "All");
  const [selectedType, setSelectedType] = useState(!isBrand ? getTypeFromSlug(slug) : "All");
  ...
  const hasActiveFilters = searchQuery !== "" || (isBrand ? false : selectedBrand !== "All") || (isBrand ? selectedType !== "All" : false);
  const clearFilters = () => { setSearchQuery(""); if (isBrand) setSelectedType("All"); else setSelectedBrand("All"); };

  const brandOptions = useMemo(() => {
    let available = selectedType !== "All" ? cars.filter((c) => c.car_type === selectedType) : cars;
    if (searchQuery.trim()) {
      const q = searchQuery.toLowerCase();
      available = available.filter((c) => c.title.toLowerCase().includes(q) || c.brand.toLowerCase().includes(q) || (c.car_type || '').toLowerCase().includes(q));
    }
    return [...new Set(available.map((c) => c.brand).filter(Boolean))].sort();
  }, [cars, searchQuery, selectedType]);

  const typeOptions = useMemo(() => {
    let available = selectedBrand !== "All" ? cars.filter((c) => c.brand === selectedBrand) : cars;
    if (searchQuery.trim()) {
      const q = searchQuery.toLowerCase();
      available = available.filter((c) => c.title.toLowerCase().includes(q) || c.brand.toLowerCase().includes(q) || (c.car_type || '').toLowerCase().includes(q));
    }
  ...
  if (loading) return (
    <div className="animate-pulse">
      <div className="p-6 md:p-8 rounded-3xl bg-white/[0.02] border border-white/[0.06]">
        ...
        <div className="h-4 w-32 bg-white/[0.04] rounded-lg mb-6" />
        <div className="flex gap-3">
          <div className="h-10 w-[200px] bg-white/[0.04] rounded-xl" />
          <div className="h-10 w-[140px] bg-white/[0.04] rounded-xl" />
          <div className="h-10 w-[130px] bg-white/[0.04] rounded-xl" />
          <div className="h-10 w-[130px] bg-white/[0.04] rounded-xl" />
        </div>
      </div>
      <div className="grid grid-auto-cars gap-4 md:gap-5 mt-8">
        {[1,2,3,4].map((i) => (
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8">
      ...
      >
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05, duration: 0.4 }}>
          <a href={locale === "en" ? "/" : "/" + locale + "/"} className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-gold transition-colors mb-3">
          ...
          <h1 className="font-serif text-3xl md:text-5xl font-bold text-white">{pageTitle}</h1>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}
            className="text-gray-500 text-sm mt-1.5">
            <span className="text-gold font-semibold">{filteredCars.length}</span> {t("carsAvailable")}
          </motion.p>
        </motion.div>

        {/* Gold Divider */}
        <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ delay: 0.12, duration: 0.6, ease: easeOut }}
      ...
      <AnimatePresence mode="wait">
        {paginatedCars.length === 0 ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
            ...
            <h3 className="text-lg font-bold text-white mb-2">{t("noCarsTitle")}</h3>
            <p className="text-gray-500 text-sm">{t("noCarsDesc")}</p>
          </motion.div>
        ) : viewMode === "grid" ? (
          ...
          <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-2">
            {paginatedCars.map((car, i) => (<ListCarItem key={car.id} car={car} index={i} fromLabel={t("from")} perDayLabel={t("perDay")} locale={locale} />))}
          </motion.div>
        )}
      </AnimatePresence>
...
const BRAND_SLUG_MAP: Record<string, string> = {
  "rent-lamborghini-in-dubai": "Lamborghini",
  "rent-ferrari-in-dubai": "Ferrari",
  "rent-rolls-royce-in-dubai": "Rolls Royce",
  "rent-bentley-in-dubai": "Bentley",
  "rent-porsche-in-dubai": "Porsche",
  "rent-mercedes-in-dubai": "Mercedes",
  "rent-bmw-in-dubai": "BMW",
  "rent-audi-in-dubai": "Audi",
  "rent-range-rover-in-dubai": "Range Rover",
  "rent-nissan-in-dubai": "Nissan",
  "rent-chevrolet-in-dubai": "Chevrolet",
  "rent-mclaren-in-dubai": "McLaren",
  "rent-cadillac-in-dubai": "Cadillac",
  "rent-gmc-in-dubai": "GMC",
  "rent-toyota-in-dubai": "Toyota",
  "rent-volkswagen-in-dubai": "Volkswagen",
  "rent-mini-cooper-in-dubai": "Mini Cooper",
  "rent-ford-in-dubai": "Ford",
};
...
function getBrandFromSlug(slug: string): string {
  return BRAND_SLUG_MAP[slug] || "";
}

function getTypeFromSlug(slug: string): string {
  return TYPE_SLUG_MAP[slug] || "";
}
